...
public class TestDom4j {
	...
	public static void  writer(Document document) throws Exception {
		...
		XMLWriter writer = new XMLWriter(new OutputStreamWriter(new FileOutputStream(new File("temp.xml")), "UTF-8"), format);
		// 写入
	...
	public static void  listNodes(Element node) {
		...
		if (!(node.getTextTrim().equals(""))) {
			...
			if (node.getText().endsWith(".ipa")) {
				System.err.println("就是你。。。。。。。");
				node.setText("找到并修改你");
				return;
			}
		}
